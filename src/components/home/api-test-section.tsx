                ...
                >
                    ...
                    <div className="flex flex-col gap-6 rounded-2xl p-6" style={{ background: "#0D0E10", border: "1px solid rgba(140,146,157,0.4)" }}>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div
                                className="rounded-lg p-4 min-h-[160px] font-geist text-sm overflow-x-auto"
                                style={{ background: "#000000", border: "1px solid rgba(140,146,157,0.4)" }}
                            >
                                {loading && (
                                    <div className="flex items-center gap-2" style={{ color: "#A9ADB5" }}>
                                        <Loader2 size={16} className="animate-spin" />
                                        <span>Fetching response…</span>
                                    </div>
                                )}
                                {!loading && response && (
                                    <pre className="text-green-400 whitespace-pre-wrap" style={{ fontSize: 13, lineHeight: "1.6em" }}>
                                        {JSON.stringify(response, null, 2)}
                                    </pre>
                                )}
                                {!loading && !response && (
                                    <p style={{ color: "#A9ADB5" }}>
                                        Hit <span className="text-orange-400 font-semibold">Test API</span> to see the response here.
                                    </p>
                                )}
                            </div>
                        </div>
